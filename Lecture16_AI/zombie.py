...
class Zombie:
    ...
    def move_slightly_to(self, tx, ty):
        if tx > 1280 - 100: tx = 1280 - 100
        if tx < 0 +100: tx = 0 +100
        if ty > 1080 - 100: ty = 1280 - 100
        if ty < 0 + 100: ty = 0 + 100
    ...
    def run_away_boy(self, r=0.5):
        self.state = 'Walk'
        if play_mode.boy.x > self.x: a = -200
        else: a = 200
        if play_mode.boy.y > self.y:
            b = -200
        else:
            b = 200
        self.move_slightly_to(self.x + a, self.y + b)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c2 = Condition("소년보다 공이 많은가",self.is_more_than_ball_boy)
        a4 = Action('소년으로 이동',self.move_to_boy)

        SEQ_chase_boy = Sequence('소년을 추적',c1,c2,a4)

        c3 = Condition("소년보다 공이 적은가", self.is_less_than_ball_boy)
        a5 = Action('소년으로부터 도망', self.run_away_boy)

        SEQ_runaway_boy = Sequence('도망', c1, c3, a5)

        root = SEL_chase_or_wander = Selector("추적 또는 도망 또는 배회",SEQ_chase_boy,SEQ_runaway_boy,SEQ_wander)
